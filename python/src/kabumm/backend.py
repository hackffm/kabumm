...
class RaspiBackend:
    def __init__(self, gpio_request, num_wires_and_pixels: int, port_number_of_pixel: list[int]):
        ...
        self.pixels = neopixel.NeoPixel(board.D18, num_wires_and_pixels, pixel_order=GRB, auto_write=False)
    ...
    def set_pixels(values):
        logger.info("set pixels to %s", values)
        self.pixels[:] = values

    def is_cut() -> list[bool]:
        gpio_lines = self.gpio_request.get_values(self.port_number_of_pixel)
        values = [gpio_lines[port] == gpiod.line.Value.ACTIVE for port in self.port_number_of_pixel]
        logger.debug("read wires: %s", "".join(hex(i)[-1] if values[i] else "_" for i in range(len(self.port_number_of_pixel))))
        return values

def _get_chip_path(self) -> str:
    candidates = []
    for i in range(10):
...
def make_backend(num_wires_and_pixels: int, port_number_of_pixel: list[int]|None=None):
    ...
    assert len(self.port_number_of_pixel) == num_wires_and_pixels
    assert len(set(self.port_number_of_pixel)) == num_wires_and_pixels, "port_number_of_pixel values must be unique"
    assert all((v <16 for v in self.port_number_of_pixel)), "port numbers must be <16"

    logger.debug("creating gpio expander chip...")
    with gpiod.Chip(path=self._get_chip_path()) as chip:
        ...
        request = selfchip.request_lines({port: settings for port in port_number_of_pixel})
        try:
            yield RaspiBackend(num_wires_and_pixels, request, port_number_of_pixel)
        finally:
            request.release()
